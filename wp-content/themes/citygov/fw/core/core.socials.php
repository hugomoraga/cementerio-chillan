<?php
/**
 * CityGov Framework: social networks
 *
 * @package	citygov
 * @since	citygov 1.0
 */
...
// Theme init
if (!function_exists('citygov_socials_theme_setup')) {
	add_action( 'citygov_action_before_init_theme', 'citygov_socials_theme_setup' );
	function citygov_socials_theme_setup() {

		if ( !is_admin() ) {
			// Add og:image meta tag for facebook
			add_action( 'wp_head', 					'citygov_facebook_og_tags', 5 );
		}
...
citygov_storage_set('share_links', array(
	...
	'postila' =>		'http://postila.ru/publish/?url={link}&agregator=citygov',
...
if (!function_exists('citygov_add_share_link')) {
	function citygov_add_share_link($soc, $url='') {
		if (!is_array($soc)) $soc = array($soc => $url);
		citygov_storage_set('share_links', array_merge( citygov_storage_get('share_links'), $soc ) );
	}
}

// Return (and show) share social links
if (!function_exists('citygov_show_share_links')) {
	function citygov_show_share_links($args) {
		if ( citygov_get_custom_option('show_share')=='hide' ) return '';
		...
		$args = array_merge(array(
			...
			'style' => citygov_get_theme_setting('socials_type')=='images' ? 'bg' : 'icons',	// style for show icons: icons|images|bg
			'type' => 'block',					// share block type: list|block|drop
			'popup' => true,					// open share url in new window or in popup window
			'counters' => citygov_get_custom_option('show_share_counters')=='yes',	// show share counters
			'direction' => citygov_get_custom_option('show_share'),				// share block direction
			'caption' => citygov_get_custom_option('share_caption'),				// share block caption
			'share' => citygov_get_theme_option('share_buttons'),					// list of allowed socials
		...
		if (is_array($args['share']) && count($args['share']) > 0) {
			foreach ($args['share'] as $soc) {
				$icon = $args['style']=='icons' || citygov_strpos($soc['icon'], $upload_url)!==false ? $soc['icon'] : citygov_get_socials_url(basename($soc['icon']));
				if ($args['style'] == 'icons') {
				...
				$url = $soc['url'];
				if (empty($url)) $url = citygov_storage_get_array('share_links', $sn);
				...
				$output .= '<div class="sc_socials_item' . (!empty($args['popup']) ? ' social_item_popup' : '') . '">'
					...
					. ($args['type']=='drop' ? '<i>' . trim(citygov_strtoproper($sn)) . '</i>' : '')
					. '</div>';
			}
		}
		$output .= '</div>';
...
if (!function_exists('citygov_prepare_socials')) {
	function citygov_prepare_socials($list, $style='') {
		if (empty($style)) $style = citygov_get_theme_setting('socials_type')=='images' ? 'bg' : 'icons';
		$output = '';
		$upload_info = wp_upload_dir();
		$upload_url = $upload_info['baseurl'];
		if (is_array($list) && count($list) > 0) {
			foreach ($list as $soc) {
				...
				$icon = $style=='icons' || citygov_strpos($soc['icon'], $upload_url)!==false ? $soc['icon'] : citygov_get_socials_url(basename($soc['icon']));
				if ($style == 'icons') {
					$parts = explode('-', $soc['icon'], 2);
					$sn = isset($parts[1]) ? $parts[1] : $parts[0];
				} else {
...
if (!function_exists('citygov_get_twitter_data')) {
	function citygov_get_twitter_data($cfg) {
...
if (!function_exists('citygov_prepare_twitter_text')) {
	function citygov_prepare_twitter_text($tweet) {
		$text = $tweet['text'];
...
if (!function_exists('citygov_get_twitter_followers')) {
	function citygov_get_twitter_followers($cfg) {
		$data = citygov_get_twitter_data($cfg); 
...
if (!function_exists('citygov_get_facebook_likes')) {
	function citygov_get_facebook_likes($account) {
		$fb = get_transient("facebooklikes");
		if ($fb !== false) return $fb;
		$fb = '?';
		$url = esc_url(citygov_get_protocol().'http://graph.facebook.com/'.($account));
		$headers = get_headers($url);
		if (citygov_strpos($headers[0], '200')) {
			$json = citygov_fgc($url);
...
if (!function_exists('citygov_facebook_og_tags')) {
	function citygov_facebook_og_tags() {
		global $post;
		if ( !is_singular() || citygov_storage_get('blog_streampage')) return;
...
if (!function_exists('citygov_get_feedburner_counter')) {
	function citygov_get_feedburner_counter($account) {
		$rss = get_transient("feedburnercounter");
		if ($rss !== false) return $rss;
		$rss = '?';
		$url = esc_url(citygov_get_protocol().'://feedburner.google.com/api/awareness/1.0/GetFeedData?uri='.($account));
		$headers = get_headers($url);
		if (citygov_strpos($headers[0], '200')) {
			$xml = citygov_fgc($url);

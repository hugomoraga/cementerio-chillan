...
jQuery(document).ready(function(){
	...
	jQuery('.to_demo_wrap').each(function(idx) {
		"use strict";
		var tab = jQuery(this).parents('.citygov_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
		var info = jQuery(this).parents('.citygov_options_field_info').attr('id', 'to_demo_scheme_'+scheme).attr('data-scheme', scheme).data('scheme', scheme);
		if (idx==0 && jQuery(this).find('.to_demo_pin').length==1 && citygov_get_cookie('to_demo_fixed')==1) citygov_options_fix_demo(info);
	});

	// Stick demo panel
	jQuery(document).on('click', '.citygov_options_field_info .to_demo_pin', function(e) {
		citygov_options_fix_demo(jQuery(this).parents('.citygov_options_field_info'));
		e.preventDefault();
		return false;
	});	

	// Update demo on page load
	citygov_options_decorate_demo();

	// Update demo on change fields
	jQuery('#partition_schemes_content').on('change', '.citygov_options_input,.citygov_options_field input[type="hidden"]', function(e) {
		"use strict";
		var tab = jQuery(this).parents('.citygov_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
		citygov_options_decorate_demo(scheme);
	});

	// Update demo on blur color fields
	jQuery('#partition_schemes_content').on('blur', '.citygov_options_input_color', function(e) {
		"use strict";
		var tab = jQuery(this).parents('.citygov_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
	...
	jQuery('#partition_schemes_content .citygov_options_tab > ul').sortable({
		update: function(event, ui) {
			var tabs = "";
			ui.item.parents('.citygov_options_tab').find('> ul > li').each(function() {
				tabs += (tabs ? CITYGOV_STORAGE['to_delimiter'] : '') + jQuery(this).attr('id').substr(4);
			});
			ui.item.parents('.citygov_options_tab').find('> .citygov_options_schemes_order').val(tabs);
		}
	}).disableSelection().after('<input type="hidden" name="citygov_options_schemes_order" class="citygov_options_schemes_order" value="">');
});


// When switch tabs - update fixed demo block
function citygov_init_hidden_elements(panel) {
	"use strict";
	var demo = panel.find('.citygov_options_field_info[data-scheme]');
	if (demo.length == 0) return;
	var info2 = jQuery('.citygov_options').find('>.citygov_options_field_info');
...
function citygov_options_action_scheme_delete(obj) {
	"use strict";
	var button = jQuery(obj);
	var tab_id = button.parents('.citygov_options_tab_content').attr('id');
	var scheme = tab_id.substring(4, tab_id.length-8);
	citygov_message_confirm(CITYGOV_STORAGE['to_strings']['scheme_delete_confirm'], CITYGOV_STORAGE['to_strings']['scheme_delete']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
			action: 'citygov_options_scheme_delete',
			nonce: CITYGOV_STORAGE['ajax_nonce'],
			scheme: scheme,
			order: button.parents('.citygov_options_tab').find('> .citygov_options_schemes_order').val()
		};
		setTimeout(function(){
			citygov_message_info(CITYGOV_STORAGE['to_strings']['recompile_styles'], CITYGOV_STORAGE['to_strings']['wait'], 'spin3 animate-spin', 60000);
		}, 600);
		jQuery.post(CITYGOV_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error === '') {
				citygov_message_success(CITYGOV_STORAGE['to_strings']['scheme_delete_complete']+'<br>'+CITYGOV_STORAGE['to_strings']['reload_page'], CITYGOV_STORAGE['to_strings']['scheme_delete']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				citygov_message_warning(CITYGOV_STORAGE['to_strings']['scheme_delete_failed']+':<br>'+rez.error, CITYGOV_STORAGE['to_strings']['scheme_delete']);
			}
		});
...
function citygov_options_action_scheme_copy(obj) {
	"use strict";
	var button = jQuery(obj);
	var tab_id = button.parents('.citygov_options_tab_content').attr('id');
	var scheme = tab_id.substring(4, tab_id.length-8);
	citygov_message_confirm(CITYGOV_STORAGE['to_strings']['scheme_copy_confirm'], CITYGOV_STORAGE['to_strings']['scheme_copy']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
			action: 'citygov_options_scheme_copy',
			nonce: CITYGOV_STORAGE['ajax_nonce'],
			scheme: scheme,
			order: button.parents('.citygov_options_tab').find('> .citygov_options_schemes_order').val()
		};
		...
		jQuery.post(CITYGOV_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error === '') {
				citygov_message_success(CITYGOV_STORAGE['to_strings']['scheme_copy_complete']+'<br>'+CITYGOV_STORAGE['to_strings']['reload_page'], CITYGOV_STORAGE['to_strings']['scheme_copy']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				citygov_message_warning(CITYGOV_STORAGE['to_strings']['scheme_copy_failed']+':<br>'+rez.error, CITYGOV_STORAGE['to_strings']['scheme_copy']);
			}
		});
...
function citygov_options_fix_demo(info) {
	var info2 = info.parents('.citygov_options').toggleClass('to_demo_fixed').find('>.citygov_options_field_info');
	if (info2.length == 0) {
		var clonedObj = info.clone();
		clonedObj.removeAttr('id');
		info.parents('.citygov_options').append(clonedObj);
		info2 = info.parents('.citygov_options').find('>.citygov_options_field_info');
	}
	citygov_set_cookie('to_demo_fixed', jQuery('.citygov_options').hasClass('to_demo_fixed') ? 1 : 0, 365);
}


// Decorate demo block
function citygov_options_decorate_demo() {
	"use strict";
	...
	jQuery('.to_demo_body').each(function() {
		...
		var scheme = jQuery(this).parents('.citygov_options_field_info').data('scheme');

...
jQuery(document).ready(function(){
	...
	jQuery('.trx_importer_section').on('click', '.trx_buttons input[type="button"]', function() {
		...
		citygov_importer_do_action(steps, 0);
	});
});

// Call specified action (step)
function citygov_importer_do_action(steps, idx) {
	"use strict";
	...
	// AJAX query params
	var data = {
		ajax_nonce: CITYGOV_STORAGE['ajax_nonce'],
		action: 'citygov_importer_start_import',
	...
	jQuery.post(CITYGOV_STORAGE['ajax_url'], data, function(response) {
		...
		try {
			...
			rez = { error: CITYGOV_STORAGE['ajax_error']+':<br>'+response };
		...
		if (rez.error === '') {
			...
			// Do next action
			if (idx < steps.length) {
				citygov_importer_do_action(steps, idx);
			} else {

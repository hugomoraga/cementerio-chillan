...
if ( !function_exists( 'citygov_template_news_announce_theme_setup' ) ) {
	add_action( 'citygov_action_before_init_theme', 'citygov_template_news_announce_theme_setup', 1 );
	function citygov_template_news_announce_theme_setup() {
		citygov_add_template(array(
			'layout' => 'news-announce',
			'template' => 'news-announce',
			'mode'   => 'news',
			'title'  => esc_html__('Recent News /Style Announce/', 'citygov')
		));

		// Add thumb sizes into list
		citygov_add_thumb_sizes( array( 'thumb_slug' => 'full', 'add_image_size' => false, 'w' => 1170, 'h' => 659 ) );
		citygov_add_thumb_sizes( array( 'thumb_slug' => 'big', 'add_image_size' => false, 'w' => 770, 'h' => 434 ) );
		citygov_add_thumb_sizes( array( 'thumb_slug' => 'medium', 'add_image_size' => false, 'w' => 770, 'h' => 217 ) );
		citygov_add_thumb_sizes( array( 'thumb_slug' => 'small', 'add_image_size' => false, 'w' => 370, 'h' => 209 ) );
	}
}

// Template output
if ( !function_exists( 'citygov_template_news_announce_output' ) ) {
	function citygov_template_news_announce_output($post_options, $post_data) {
		...
		$post_data['post_thumb'] = citygov_get_resized_image_tag($post_data['post_attachment'], $thumb_sizes['w'], $post_data['post_type']=='product' && citygov_get_theme_option('crop_product_thumb')=='no' ? null :  $thumb_sizes['h']);

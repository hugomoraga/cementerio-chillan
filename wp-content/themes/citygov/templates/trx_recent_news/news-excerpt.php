...
if ( !function_exists( 'citygov_template_news_excerpt_theme_setup' ) ) {
	add_action( 'citygov_action_before_init_theme', 'citygov_template_news_excerpt_theme_setup', 1 );
	function citygov_template_news_excerpt_theme_setup() {
		citygov_add_template(array(
			'layout' => 'news-excerpt',
			'template' => 'news-excerpt',
			'mode'   => 'news',
			'title'  => esc_html__('Recent News /Style Excerpt/', 'citygov'),
...
// Template output
if ( !function_exists( 'citygov_template_news_excerpt_output' ) ) {
	function citygov_template_news_excerpt_output($post_options, $post_data) {
						...
						if ($post_data['post_excerpt']) {
							echo in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status')) 
									...
									: wpautop(citygov_strshort($post_data['post_excerpt'], isset($post_options['descr']) 
																								? $post_options['descr'] 
																								: citygov_get_custom_option('post_excerpt_maxlength')
																)
											);
						}
					}

...
if ( !function_exists( 'citygov_template_expert_theme_setup' ) ) {
	add_action( 'citygov_action_before_init_theme', 'citygov_template_expert_theme_setup', 1 );
	function citygov_template_expert_theme_setup() {
		citygov_add_template(array(
			'layout' => 'expert',
			'mode'   => 'blog',
			'title'  => esc_html__('Expert', 'citygov'),
			'thumb_title'  => esc_html__('Largest image (crop)', 'citygov'),
			'w'		 => 468,
			'h'		 => 408
		));
	}
...
// Template output
if ( !function_exists( 'citygov_template_expert_output' ) ) {
	function citygov_template_expert_output($post_options, $post_data) {
		$show_title = true;

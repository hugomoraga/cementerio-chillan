...
if ( !function_exists( 'citygov_template_masonry_theme_setup' ) ) {
	add_action( 'citygov_action_before_init_theme', 'citygov_template_masonry_theme_setup', 1 );
	function citygov_template_masonry_theme_setup() {
		citygov_add_template(array(
			'layout' => 'masonry_2',
			'template' => 'masonry',
			'mode'   => 'blog',
			'need_isotope' => true,
			'title'  => esc_html__('Masonry tile (different height) /2 columns/', 'citygov'),
		...
		citygov_add_template(array(
			...
			'mode'   => 'blog',
			'need_isotope' => true,
			'title'  => esc_html__('Masonry tile /3 columns/', 'citygov'),
			'thumb_title'  => esc_html__('Medium image', 'citygov'),
			'w'		 => 370,
			'h_crop' => 209,
			'h'      => null
		...
		citygov_add_template(array(
			...
			'mode'   => 'blog',
			'need_isotope' => true,
			'title'  => esc_html__('Masonry tile /4 columns/', 'citygov'),
			'thumb_title'  => esc_html__('Medium image', 'citygov'),
			'w'		 => 370,
			'h_crop' => 209,
			'h'      => null
		...
		add_action('citygov_action_blog_scripts', 'citygov_template_masonry_add_scripts');
	}
}

// Add template specific scripts
if (!function_exists('citygov_template_masonry_add_scripts')) {
	function citygov_template_masonry_add_scripts($style) {
		if (in_array(citygov_substr($style, 0, 8), array('masonry_'))) {
...
// Template output
if ( !function_exists( 'citygov_template_masonry_output' ) ) {
	function citygov_template_masonry_output($post_options, $post_data) {
					...
					<div class="post_descr">
						...
						if ($post_data['post_protected']) {
							...
							if ($post_data['post_excerpt']) {
								echo in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status')) ? $post_data['post_excerpt'] : '<p>'.trim(citygov_strshort($post_data['post_excerpt'], isset($post_options['descr']) ? $post_options['descr'] : citygov_get_custom_option('post_excerpt_maxlength_masonry'))).'</p>';
							}

...
if ( !function_exists( 'citygov_template_services_3_theme_setup' ) ) {
	add_action( 'citygov_action_before_init_theme', 'citygov_template_services_3_theme_setup', 1 );
	function citygov_template_services_3_theme_setup() {
		citygov_add_template(array(
			'layout' => 'services-3',
			'template' => 'services-3',
			'mode'   => 'services',
			'need_columns' => true,
			'title'  => esc_html__('Services /Style 3/', 'citygov'),
...
// Template output
if ( !function_exists( 'citygov_template_services_3_output' ) ) {
	function citygov_template_services_3_output($post_options, $post_data) {
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
				...
				if ($post_data['post_icon'] && $post_options['tag_type']=='icons') {
					echo trim(citygov_do_shortcode('[trx_icon icon="'.esc_attr($post_data['post_icon']).'" shape="round"]'));
				} else {
					?>

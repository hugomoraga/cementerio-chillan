...
if ( !function_exists( 'citygov_template_services_5_theme_setup' ) ) {
	add_action( 'citygov_action_before_init_theme', 'citygov_template_services_5_theme_setup', 1 );
	function citygov_template_services_5_theme_setup() {
		citygov_add_template(array(
			'layout' => 'services-5',
			'template' => 'services-5',
			'mode'   => 'services',
			'need_columns' => true,
			'title'  => esc_html__('Services /Style 5/', 'citygov'),
...
// Template output
if ( !function_exists( 'citygov_template_services_5_output' ) ) {
	function citygov_template_services_5_output($post_options, $post_data) {
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
				...
				if ($post_data['post_icon'] && $post_options['tag_type']=='icons') {
					$html = citygov_do_shortcode('[trx_icon icon="'.esc_attr($post_data['post_icon']).'" shape="round"]');
				...
				<div class="sc_services_item_content">
					...
					<div class="sc_services_item_description">
						...
						if ($post_data['post_protected']) {
							...
							if (!empty($post_data['post_link']) && !citygov_param_is_off($post_options['readmore'])) {

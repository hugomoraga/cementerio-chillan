...
if ( !function_exists( 'citygov_template_team_2_theme_setup' ) ) {
	add_action( 'citygov_action_before_init_theme', 'citygov_template_team_2_theme_setup', 1 );
	function citygov_template_team_2_theme_setup() {
		citygov_add_template(array(
			'layout' => 'team-2',
			'template' => 'team-2',
			'mode'   => 'team',
			'title'  => esc_html__('Team /Style 2/', 'citygov'),
...
// Template output
if ( !function_exists( 'citygov_template_team_2_output' ) ) {
	function citygov_template_team_2_output($post_options, $post_data) {
		$show_title = true;
		...
		?>
			...
			><div class="sc_team_item_avatar column-1_2">
				...
				<div class="sc_team_item_description"><?php echo trim(citygov_strshort($post_data['post_excerpt'], isset($post_options['descr']) ? $post_options['descr'] : citygov_get_custom_option('post_excerpt_maxlength_masonry'))); ?></div>

...
if ( !function_exists( 'citygov_template_classic_theme_setup' ) ) {
	add_action( 'citygov_action_before_init_theme', 'citygov_template_classic_theme_setup', 1 );
	function citygov_template_classic_theme_setup() {
		citygov_add_template(array(
			'layout' => 'classic_2',
			'template' => 'classic',
			'mode'   => 'blog',
			'need_isotope' => true,
			'title'  => esc_html__('Classic tile (equal height) /2 columns/', 'citygov'),
		...
		citygov_add_template(array(
			...
			'mode'   => 'blog',
			'need_isotope' => true,
			'title'  => esc_html__('Classic tile /3 columns/', 'citygov'),
			'thumb_title'  => esc_html__('Medium image (crop)', 'citygov'),
			'w'		 => 740,
			'h'		 => 524
		));
		citygov_add_template(array(
			...
			'mode'   => 'blog',
			'need_isotope' => true,
			'title'  => esc_html__('Classic tile /4 columns/', 'citygov'),
			'thumb_title'  => esc_html__('Medium image (crop)', 'citygov'),
			'w'		 => 740,
			'h'		 => 524
		));
		// Add template specific scripts
		add_action('citygov_action_blog_scripts', 'citygov_template_classic_add_scripts');
	}
}

// Add template specific scripts
if (!function_exists('citygov_template_classic_add_scripts')) {
	function citygov_template_classic_add_scripts($style) {
		if (in_array(citygov_substr($style, 0, 8), array('classic_'))) {
...
// Template output
if ( !function_exists( 'citygov_template_classic_output' ) ) {
	function citygov_template_classic_output($post_options, $post_data) {

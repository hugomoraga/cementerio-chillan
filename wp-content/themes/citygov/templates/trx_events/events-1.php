...
if ( !function_exists( 'citygov_template_events_1_theme_setup' ) ) {
	add_action( 'citygov_action_before_init_theme', 'citygov_template_events_1_theme_setup', 1 );
	function citygov_template_events_1_theme_setup() {
		citygov_add_template(array(
			'layout' => 'events-1',
			'template' => 'events-1',
			'mode'   => 'events',
			'title'  => esc_html__('Events /Style 1/', 'citygov'),
			'thumb_title'  => esc_html__('Middle image()', 'citygov'),
			'w'		 => 370,
			'h'		 => 262
		));
	}
...
// Template output
if ( !function_exists( 'citygov_template_events_1_output' ) ) {
	function citygov_template_events_1_output($post_options, $post_data) {
		$show_title = true;
							...
							if ($post_data['post_excerpt']) {
								echo in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status')) 
										...
										: '<p>'.trim(citygov_strshort($post_data['post_excerpt'], isset($post_options['descr']) 
																										? $post_options['descr'] 
																										: citygov_get_custom_option('post_excerpt_maxlength_masonry'))).'</p>';
							}

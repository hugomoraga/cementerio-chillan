...
if ( !function_exists( 'citygov_template_list_theme_setup' ) ) {
	add_action( 'citygov_action_before_init_theme', 'citygov_template_list_theme_setup', 1 );
	function citygov_template_list_theme_setup() {
		citygov_add_template(array(
			'layout' => 'list',
			'mode'   => 'blogger',
			'need_columns' => true,
			'title'  => esc_html__('Blogger layout: List', 'citygov')
			));
	}
}

// Template output
if ( !function_exists( 'citygov_template_list_output' ) ) {
	function citygov_template_list_output($post_options, $post_data) {

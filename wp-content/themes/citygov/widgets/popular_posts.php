...
// Theme init
if (!function_exists('citygov_widget_popular_posts_theme_setup')) {
	add_action( 'citygov_action_before_init_theme', 'citygov_widget_popular_posts_theme_setup', 1 );
	function citygov_widget_popular_posts_theme_setup() {

		// Register shortcodes in the shortcodes list
		//add_action('citygov_action_shortcodes_list',	'citygov_widget_popular_posts_reg_shortcodes');
		if (function_exists('citygov_exists_visual_composer') && citygov_exists_visual_composer())
			add_action('citygov_action_shortcodes_list_vc','citygov_widget_popular_posts_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('citygov_widget_popular_posts_load')) {
	add_action( 'widgets_init', 'citygov_widget_popular_posts_load' );
	function citygov_widget_popular_posts_load() {
		register_widget('citygov_widget_popular_posts');
	}
}

// Widget Class
class citygov_widget_popular_posts extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_popular_posts', 'description' => esc_html__('The most popular and most commented blog posts (extended)', 'citygov'));
		parent::__construct( 'citygov_widget_popular_posts', esc_html__('CityGov - Most Popular & Commented Posts', 'citygov'), $widget_ops );
	...
	function widget($args, $instance) {
		...
		$show_counters = isset($instance['show_counters']) && $instance['show_counters'] > 0 ? citygov_get_theme_option('blog_counters') : '';
		...
		for ($i=0; $i<3; $i++) {
			...
			if ($i==0) {			// Most popular
				$args['meta_key'] = 'citygov_post_views_count';
				$args['orderby'] = 'meta_value_num';
				$show_counters = $show_counters ? 'views' : '';
			} else if ($i==2) {		// Most liked
				$args['meta_key'] = 'citygov_post_likes_count';
				$args['orderby'] = 'meta_value_num';
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_popular')); ?>"><?php esc_html_e('Most popular tab title:', 'citygov'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_commented')); ?>"><?php esc_html_e('Most commented tab title:', 'citygov'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_liked')); ?>"><?php esc_html_e('Most liked tab title:', 'citygov'); ?></label>
...
*/
if ( !function_exists( 'citygov_sc_widget_popular_posts' ) ) {
	function citygov_sc_widget_popular_posts($atts, $content=null){	
		...
		extract($atts);
		$type = 'citygov_widget_popular_posts';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_popular_posts' 
								. (citygov_exists_visual_composer() ? ' vc_widget_popular_posts wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, citygov_prepare_widgets_args(citygov_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_popular_posts', 'widget_popular_posts') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('citygov_shortcode_output', $output, 'trx_widget_popular_posts', $atts, $content);
	}
	citygov_require_shortcode("trx_widget_popular_posts", "citygov_sc_widget_popular_posts");
}


// Add [trx_widget_popular_posts] in the VC shortcodes list
if (!function_exists('citygov_widget_popular_posts_reg_shortcodes_vc')) {
	//add_action('citygov_action_shortcodes_list_vc','citygov_widget_popular_posts_reg_shortcodes_vc');
	function citygov_widget_popular_posts_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Widget Popular Posts", "citygov"),
				"description" => wp_kses_data( __("Insert popular posts list with thumbs and post's meta", "citygov") ),
				...
				"params" => array(
					...
					array(
						...
						"heading" => esc_html__("Most popular tab title", "citygov"),
						"description" => wp_kses_data( __("Most popular tab title", "citygov") ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
					array(
						...
						"heading" => esc_html__("Most commented tab title", "citygov"),
						"description" => wp_kses_data( __("Most commented tab title", "citygov") ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
					array(
						...
						"heading" => esc_html__("Most liked tab title", "citygov"),
						"description" => wp_kses_data( __("Most liked tab title", "citygov") ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),

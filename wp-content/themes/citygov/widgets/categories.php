...
// Theme init
if (!function_exists('citygov_widget_categories_theme_setup')) {
	add_action( 'citygov_action_before_init_theme', 'citygov_widget_categories_theme_setup', 1 );
	function citygov_widget_categories_theme_setup() {

		// Register shortcodes in the shortcodes list
		//add_action('citygov_action_shortcodes_list',		'citygov_widget_categories_reg_shortcodes');
		if (function_exists('citygov_exists_visual_composer') && citygov_exists_visual_composer())
			add_action('citygov_action_shortcodes_list_vc','citygov_widget_categories_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('citygov_widget_categories_load')) {
	add_action( 'widgets_init', 'citygov_widget_categories_load' );
	function citygov_widget_categories_load() {
		register_widget( 'citygov_widget_categories' );
	}
}

// Widget Class
class citygov_widget_categories extends WP_Widget {

	function __construct() {
		$widget_ops = array( 'classname' => 'widget_subcategories', 'description' => esc_html__('Display subcategories list', 'citygov') );
		parent::__construct( 'citygov_widget_subcategories', esc_html__('CityGov - Subcategories list', 'citygov'), $widget_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		<div class="widget_subcategories_inner">
			<?php
			if ( $d ) {
				$cat_args['show_option_none'] = esc_html__('Select Category', 'citygov');
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('root')); ?>"><?php esc_html_e('Root category:', 'citygov'); ?></label>
		...
		<label for="<?php echo esc_attr($this->get_field_id('dropdown')); ?>"><?php esc_html_e( 'Display as dropdown', 'citygov' ); ?></label><br />

		<input type="checkbox" class="checkbox" id="<?php echo esc_attr($this->get_field_id('count')); ?>" name="<?php echo esc_attr($this->get_field_name('count')); ?>"<?php checked( $count ); ?> />
		<label for="<?php echo esc_attr($this->get_field_id('count')); ?>"><?php esc_html_e( 'Show post counts', 'citygov' ); ?></label><br />

		<input type="checkbox" class="checkbox" id="<?php echo esc_attr($this->get_field_id('hierarchical')); ?>" name="<?php echo esc_attr($this->get_field_name('hierarchical')); ?>"<?php checked( $hierarchical ); ?> />
		<label for="<?php echo esc_attr($this->get_field_id('hierarchical')); ?>"><?php esc_html_e( 'Show hierarchy', 'citygov' ); ?></label>
		</p>
		<?php
	}
}
...
*/
if ( !function_exists( 'citygov_sc_widget_categories' ) ) {
	function citygov_sc_widget_categories($atts, $content=null){	
		...
		extract($atts);
		$type = 'citygov_widget_categories';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_categories' 
								. (citygov_exists_visual_composer() ? ' vc_widget_categories wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, citygov_prepare_widgets_args(citygov_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_categories', 'widget_categories') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('citygov_shortcode_output', $output, 'trx_widget_categories', $atts, $content);
	}
	citygov_require_shortcode("trx_widget_categories", "citygov_sc_widget_categories");
}


// Add [trx_widget_categories] in the VC shortcodes list
if (!function_exists('citygov_widget_categories_reg_shortcodes_vc')) {
	//add_action('citygov_action_shortcodes_list_vc','citygov_widget_categories_reg_shortcodes_vc');
	function citygov_widget_categories_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Widget Categories", "citygov"),
				"description" => wp_kses_data( __("Display the subcategories list for the specified category", "citygov") ),
				...
				"params" => array(
					...
					array(
						"param_name" => "count",
						"heading" => esc_html__("Show posts", "citygov"),
						"description" => wp_kses_data( __("Show posts number in the each category", "citygov") ),
						"class" => "",
						"std" => "1",
						"value" => array(esc_html__('Show posts number', 'citygov') => "1"),
						"type" => "checkbox"
					),
					array(
						"param_name" => "dropdown",
						"heading" => esc_html__("Show dropdown", "citygov"),
						"description" => wp_kses_data( __("Show categories as dropdown list", "citygov") ),
						"class" => "",
						"std" => "0",
						"value" => array(esc_html__('Show dropdown', 'citygov') => "1"),
						"type" => "checkbox"
					),
					array(
						"param_name" => "hierarchical",
						"heading" => esc_html__("Show hierarchical", "citygov"),
						"description" => wp_kses_data( __("Show categories as hierarchical list", "citygov") ),
						"class" => "",
						"std" => "1",
						"value" => array(esc_html__('Show hierarchical', 'citygov') => "1"),
						"type" => "checkbox"
					),
					array(
					...
					array(
						...
						"description" => wp_kses_data( __("Select parent category. If empty - show all categories", "citygov") ),
						"class" => "",

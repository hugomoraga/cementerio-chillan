...
// Theme init
if (!function_exists('citygov_widget_recent_news_theme_setup')) {
	add_action( 'citygov_action_before_init_theme', 'citygov_widget_recent_news_theme_setup', 1 );
	function citygov_widget_recent_news_theme_setup() {

		// Register shortcodes in the shortcodes list
		//add_action('citygov_action_shortcodes_list',	'citygov_widget_recent_news_reg_shortcodes');
		if (function_exists('citygov_exists_visual_composer') && citygov_exists_visual_composer())
			add_action('citygov_action_shortcodes_list_vc','citygov_widget_recent_news_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('citygov_widget_recent_news_load')) {
	add_action( 'widgets_init', 'citygov_widget_recent_news_load' );
	function citygov_widget_recent_news_load() {
		register_widget('citygov_widget_recent_news');
	}
}


// Widget Class
class citygov_widget_recent_news extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_news', 'description' => esc_html__('Show recent news in many styles', 'citygov'));
		parent::__construct( 'citygov_widget_recent_news', esc_html__('CityGov - Recent News', 'citygov'), $widget_ops );
	}
	...
	function widget($args, $instance) {
		...
		$output = citygov_sc_recent_news( array(
			'from_widget'		=> true,
	...
	function form($instance) {
		...
		$news_styles = citygov_get_list_templates('news');
		?>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('widget_title')); ?>"><?php esc_html_e('Widget title:', 'citygov'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Block title:', 'citygov'); ?></label>
			<input id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" value="<?php echo esc_attr($title); ?>" class="widgets_param_fullwidth" />
		</p>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('subtitle')); ?>"><?php esc_html_e('Block subtitle:', 'citygov'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('style')); ?>"><?php esc_html_e('Style:', 'citygov'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('count')); ?>"><?php esc_html_e('Number of posts to be displayed:', 'citygov'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('featured')); ?>"><?php esc_html_e('Number of featured posts:', 'citygov'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('columns')); ?>"><?php esc_html_e('Number of columns:', 'citygov'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_categories')); ?>_1"><?php esc_html_e('Show categories:', 'citygov'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_categories')); ?>_1"><?php esc_html_e('Show', 'citygov'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_categories')); ?>_0"><?php esc_html_e('Hide', 'citygov'); ?></label>
		</p>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_1"><?php esc_html_e('Show counters:', 'citygov'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_1"><?php esc_html_e('Show', 'citygov'); ?></label>
...
*/
if ( !function_exists( 'citygov_sc_recent_news' ) ) {
	function citygov_sc_recent_news($atts, $content=null){	
		...
		// Get categories list
		if (citygov_param_is_on($show_categories)) {
			if ( !citygov_storage_isset('categories_'.$category) ) {
				citygov_storage_set('categories_'.$category, get_categories(array(
					'orderby' => 'name',
					'parent' => $category
					))
				);
		...
		if (empty($from_widget)) {
			$widget_args = citygov_prepare_widgets_args(citygov_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_news', 'widget_recent_news');
			$output .= '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_recent_news_wrap' 
								. (citygov_exists_visual_composer() ? ' vc_recent_news wpb_content_element' : '') 
		...
		// Header
		if ( !empty($title) || !empty($subtitle) || (citygov_param_is_on($show_categories) && !citygov_storage_empty('categories_'.$category)) ) {
			$output	.= '<div class="sc_recent_news_header'.(citygov_param_is_on($show_categories) && !citygov_storage_empty('categories_'.$category) ? ' sc_recent_news_header_split' : '').'">'
			...
			// Categories list
			$cat_list = citygov_storage_get('categories_'.$category);
			if (citygov_param_is_on($show_categories) && !empty($cat_list)) {
				...
				if (is_array($cat_list) && count($cat_list) > 0) {
					$output .= '<a href="' . esc_url( $category == 0 
						...
						: get_category_link($category) ) . '" class="sc_recent_news_header_category_item">'.esc_html__('All News', 'citygov').'</span>';
					$number = 0;
					$number_max = 3;
					foreach ($cat_list as $cat) {
						$number++;
						if ($number == $number_max)
							$output .= '<span class="sc_recent_news_header_category_item sc_recent_news_header_category_item_more">'.esc_html__('More', 'citygov')
		...
		$args = citygov_query_add_posts_and_cats($args, $ids, $post_type, $category, $taxonomy);
		...
		return apply_filters('citygov_shortcode_output', $output, 'trx_recent_news', $atts, $content);
	}
	citygov_require_shortcode("trx_recent_news", "citygov_sc_recent_news");
}


// Add [trx_recent_news] in the VC shortcodes list
if (!function_exists('citygov_widget_recent_news_reg_shortcodes_vc')) {
	//add_action('citygov_action_shortcodes_list_vc','citygov_widget_recent_news_reg_shortcodes_vc');
	function citygov_widget_recent_news_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Recent News", "citygov"),
				"description" => wp_kses_data( __("Insert recent news list", "citygov") ),
				...
				"params" => array(
					array(
						...
						"description" => wp_kses_data( __("Select style to display news list", "citygov") ),
						"class" => "",
						"admin_label" => true,
						"std" => 'news-magazine',
					...
					array(
						...
						"description" => wp_kses_data( __("Title for the widget (fill this field only if you want to use shortcode as widget)", "citygov") ),
						"admin_label" => true,
						"class" => "",
						"value" => "",
						"type" => "textfield"
					...
					array(
						...
						"heading" => esc_html__("Show categories", "citygov"),
						"description" => wp_kses_data( __("Show categories in the shortcode's header", "citygov") ),
						"class" => "",
					...
					array(
						...
						"heading" => esc_html__("Show counters", "citygov"),
						"description" => wp_kses_data( __("Show counters (comments and views) in the shortcode's footer", "citygov") ),
						'dependency' => array(
							'element' => 'style',
							'value' => 'news-magazine'
						),
					...
					array(
						...
						"description" => wp_kses_data( __("Select category to show news. If empty - select news from any category or from IDs list", "citygov") ),
						"group" => esc_html__('Query', 'citygov'),
						"class" => "",
					...
					array(
						"param_name" => "count",
						"heading" => esc_html__("Total posts", "citygov"),
					...
					array(
						"param_name" => "featured",
						"heading" => esc_html__("Featured posts", "citygov"),
						"description" => wp_kses_data( __("How many posts will be displayed as featured?", "citygov") ),
						"admin_label" => true,
						"group" => esc_html__('Query', 'citygov'),
						'dependency' => array(
							'element' => 'style',
					...
					array(
						...
						"description" => wp_kses_data( __("How many columns use to show news list", "citygov") ),
						"group" => esc_html__('Query', 'citygov'),
						"admin_label" => true,
						'dependency' => array(
							'element' => 'style',
					...
					array(
						...
						"group" => esc_html__('Query', 'citygov'),
						"class" => "",
						"value" => array_flip(citygov_get_list_sortings()),
						"type" => "dropdown"
					),
					array(
						"param_name" => "order",
						...
						"group" => esc_html__('Query', 'citygov'),
						"class" => "",
						"value" => array_flip(citygov_get_list_orderings()),
						"type" => "dropdown"
					),
					array(
						"param_name" => "ids",
						"heading" => esc_html__("List IDs", "citygov"),
						"description" => wp_kses_data( __("Comma separated list of post's ID. If set - parameters above (category, count, order, etc.) are ignored!", "citygov") ),
						"group" => esc_html__('Query', 'citygov'),
						"class" => "",
						"value" => "",
						"type" => "textfield"

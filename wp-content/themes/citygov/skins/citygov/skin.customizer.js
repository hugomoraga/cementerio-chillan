...
function citygov_skin_customizer(option, val) {
	...
	if (option == 'body_style') {
		...
		var params = CITYGOV_STORAGE['co_add_params']!=undefined ? CITYGOV_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#citygov-customizer-styles-'+option).length > 0 ? jQuery('#citygov-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="citygov-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
